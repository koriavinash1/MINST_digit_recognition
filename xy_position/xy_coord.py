...
with tf.device("/cpu:0"):
    ...
    tf.summary.histogram("wc1_v", weights['wc1_v'])
    ...
    if IS_POSITION_BASED:
        ...
        tf.summary.histogram("wc1_xpos", weights['wc1_xpos'])
        tf.summary.histogram("wc1_ypos", weights['wc1_ypos'])
    ...
    tf.summary.histogram("bc1_v", biases['bc1_v'])
    ...
    if IS_POSITION_BASED:
        ...
        tf.summary.histogram("bc1_xpos", biases['bc1_xpos'])
        tf.summary.histogram("bc1_ypos", biases['bc1_ypos'])
...
with tf.Session() as sess:
    ...
    while step <= epochs * batch_size:
        ...
        if step % display_steps == 0:
            ...
            if IS_POSITION_BASED:
                summary, loss, acc = sess.run([merged, cost, accuracy], feed_dict={x: batch_x, y: batch_y, X_pos: np.array([xpos,]*batch_size, ndmin=2), Y_pos: np.array([ypos,]*batch_size, ndmin=2)})
            else:
    ...
    save(sess.run(weights['wc1_v']), "./weights/wc1_v")
    save(sess.run(weights['wc2']), "./weights/wc2")
    save(sess.run(weights['wfc1']), "./weights/wfc1")
    save(sess.run(weights['wfc2']), "./weights/wfc2")
    save(sess.run(weights['out']), "./weights/out")

    if IS_POSITION_BASED:
    	save(sess.run(weights['wc1_xpos']), "./weights/wc1_xpos")
    	save(sess.run(weights['wc1_ypos']), "./weights/wc1_ypos")

    save(sess.run(biases['bc1_v']), "./biases/bc1_v")
    save(sess.run(biases['bc2']), "./biases/bc2")
    save(sess.run(biases['bfc1']), "./biases/bfc1")
    save(sess.run(biases['bfc2']), "./biases/bfc2")
    save(sess.run(biases['out']), "./biases/out")

    if IS_POSITION_BASED:
    	save(sess.run(biases['bc1_xpos']), "./biases/bc1_xpos")
    	save(sess.run(biases['bc1_ypos']), "./biases/bc1_ypos")
